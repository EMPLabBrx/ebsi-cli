...
async function ledgerGet(inputs: string[], context: Context) {
  const apiUrl = context.config.api.ledger.url;
...
async function ledgerGetTransactionCount(
  inputs: string[],
  useProxy: boolean,
  context: Context
) {
...
async function ledgerGetChainId(
  inputs: string[],
  useProxy: boolean,
  context: Context
) {
...
async function ledgerGetTransaction(
  inputs: string[],
  useProxy: boolean,
  context: Context
) {
...
async function ledgerGetTransactionReceipt(
  inputs: string[],
  useProxy: boolean,
  context: Context
) {
...
// TODO: do not export. And instead of this export the
// one in ledger-v4.ts when this api is available in
// all environments
export async function waitToBeMined(
  ...
  while (!mined && diffTime <= context.config.timeoutMining) {
    ...
    console.log(
      `==> ${useProxy ? "proxyledger" : "ledger"} getTransactionReceipt ${txId}`
    );
...
async function ledgerSendTransaction(
  inputs: string[],
  useProxy: boolean,
  context: Context
) {
...
async function ledgerCallSmartContract(
  contractName: string,
  ...
  // build, sign and send
  const cmd = useProxy ? "proxyledger" : "ledger";
...
export async function ledgerV3(
  method: string,
  inputs: (string | UnknownObject)[],
  useProxy: boolean,
  context: Context
