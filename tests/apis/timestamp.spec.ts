...
describe("Timestamp (e2e)", () => {
  ...
  describe("GET /hash-algorithms", () => {
    it("should get a collection of hash algorithms", async () => {
      expect.assertions(2);
      vi.resetAllMocks();
      await execCommand("timestamp get /hash-algorithms");
  ...
  describe("GET /timestamps", () => {
    ...
    it("should get a collection of timestamps", async () => {
      expect.assertions(2);
      vi.resetAllMocks();
      await execCommand("timestamp get /timestamps");
    ...
    it("should get a timestamp", async () => {
      expect.assertions(2);
      vi.resetAllMocks();
      await execCommand(`timestamp get /timestamps/${timestampId}`);
      expectResponse(mockConsole, {
  ...
  describe("GET /records", () => {
    ...
    it("should get a collection of records", async () => {
      expect.assertions(3);
      vi.resetAllMocks();
      await execCommand("timestamp get /records");
      const data = expectCollection<{
        recordId: string;
        href: string;
      }>(mockConsole);
    ...
    it("should get a record", async () => {
      expect.assertions(3);
      vi.resetAllMocks();
      await execCommand(`timestamp get /records/${recordId}`);
      expectResponse(
        mockConsole,
    ...
    it("should get the versions of a record", async () => {
      expect.assertions(3);
      vi.resetAllMocks();
      await execCommand(`timestamp get /records/${recordId}/versions`);
      const data = expectCollection<{
        versionId: string;
        href: string;
      }>(mockConsole);
    ...
    it("should resolve a timestamp by its version id", async () => {
      expect.assertions(3);
      vi.resetAllMocks();
      await execCommand(
        `timestamp get /records/${recordId}/versions/${versionId}`
      );
      expectResponse(
        mockConsole,
        expect.objectContaining({
    ...
    describe.each(commandsAdmin)(
      ...
      (method: string, params: () => string) => {
        ...
        it("should reject not an administrator", async () => {
          ...
          await expect(
            execCommand(`timestamp ${method} ${params()}`)
          ).rejects.toThrow();
    ...
    describe.each(commandsAdmin)(
      ...
      (method: string, params: () => string) => {
        ...
        it("should work", async () => {
          expect.assertions(7);
          vi.resetAllMocks();
          await execCommand(`timestamp ${method} ${params()}`);
