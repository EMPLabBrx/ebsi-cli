...
const { user1, issuer1 } = config.vitestNew;
let newUser: Client;
...
describe("DID Registry (e2e)", () => {
  ...
  describe("GET /identifiers", () => {
    it("should get a collection of identifiers", async () => {
      expect.assertions(1);
      vi.resetAllMocks();
      const result = await execCommand("did-new get /identifiers");
    ...
    it("should get a did", async () => {
      expect.assertions(3);
      vi.resetAllMocks();
      await execCommand(`did-new get /identifiers/${user1.did}`);
    ...
    describe.each(commandsUser)(
      ...
      (method: string, params: () => string) => {
        it("should work", async () => {
          ...
          vi.resetAllMocks();
          await execCommand(`did-new ${method} ${params()}`);
