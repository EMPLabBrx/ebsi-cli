...
describe("Accredit and Authorize", () => {
  ...
  it("should register the DID document", async () => {
    ...
    await execCommand("run registerDidDocument_ES256K_ES256 vcOnboard.vc");
    const didDocument = await execCommand("did get /identifiers/ myIssuer.did");
    ...
    const check = await execCommand("conformance check ti_register_did");
    expect(check).toStrictEqual({ success: true });
  });
  ...
  it("should request VerifiableAccreditationToAttest (TI)", async () => {
    ...
    await execCommand(
      `set vcTI.url ${config.api.tir.url}/issuers/ myIssuer.did /attributes/ vcTI.reservedAttributeId`
    );
  ...
  it("should register the VerifiableAccreditationToAttest in the TIR", async () => {
    ...
    const attribute = await execCommand(
      "tir get /issuers/ myIssuer.did /attributes/ vcTI.reservedAttributeId"
    );
  ...
  it("should register a proxy in the Trusted Issuers Registry", async () => {
    ...
    const value = "0";
    await execCommand(
      `conformance clientMockUpdateList ${id} ${statusListId} ${value}`
    );
    await execCommand("t: authorisation auth tir_write_presentation ES256");
    await execCommand("using token t.access_token");
    ...
    // register proxy id
    await execCommand(`tir addIssuerProxy myIssuer.did ${proxyData}`);
    const response = await execCommand(
      "tir get /issuers/ myIssuer.did /proxies"
    );
    ...
    expect(response).toStrictEqual({
      items: [
        {
          proxyId,
          href: `${config.domain}/trusted-issuers-registry/v4/issuers/${myIssuer.did}/proxies/${proxyId}`,
        },
      ],
      total: 1,
    });
  ...
  it("should issue a CTRevocable credential", async () => {
    ...
    const check = await execCommand("conformance check ti_request_ctrevocable");
    expect(check).toStrictEqual({ success: true });
  });
  ...
  it("should revoke CTRevocable credential", async () => {
    ...
    await execCommand(
      `statusListIndex: compute statusListIndex ${config.api.conformance.did}`
    );
    await execCommand("conformance clientMockUpdateList 1 statusListIndex 1");

    const check = await execCommand("conformance check ti_revoke_ctrevocable");
    expect(check).toStrictEqual({ success: true });
  });
  ...
  it("should request VerifiableAccreditationToAccredit (TAO)", async () => {
    ...
    await execCommand(
      `set vcTAO.url ${config.api.tir.url}/issuers/ myIssuer.did /attributes/ vcTAO.reservedAttributeId`
    );
  ...
  it("should register the VerifiableAccreditationToAccredit in the TIR", async () => {
    ...
    await execCommand("conformance clientMockInitiate");
    const attribute = await execCommand(
      "tir get /issuers/ myIssuer.did /attributes/ vcTAO.reservedAttributeId"
    );
    expect(attribute).toStrictEqual({
      did: myIssuer.did,
      attribute: {
  ...
  it("should revoke credentials issued to subaccount", async () => {
    ...
    await execCommand("attributes: tir get /issuers/ subaccount /attributes");
    await execCommand("t: authorisation auth tir_write_presentation ES256");
    await execCommand("using token t.access_token");
    await execCommand(
  ...
  it("should request VerifiableAuthorisationForTrustChain (RootTAO)", async () => {
    ...
    await execCommand(
      `set vcRootTAO.url ${config.api.tir.url}/issuers/ myIssuer.did /attributes/ vcRootTAO.reservedAttributeId`
    );
  ...
  it("should register the VerifiableAuthorisationForTrustChain in the TIR", async () => {
    ...
    const attribute = await execCommand(
      "tir get /issuers/ myIssuer.did /attributes/ vcRootTAO.reservedAttributeId"
    );
    expect(attribute).toStrictEqual({
      did: myIssuer.did,
      attribute: {
